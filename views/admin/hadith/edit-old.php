...
//		echo '<pre>'; var_dump($hadithLangInfo[0]); echo '</pre>';
					...
					<form class="form-row" method="POST" enctype="multipart/form-data">
						...
						<div class="col-12 mt-4">
							...
							<div class="tab-content pt-2" id="myTabContent">
								<?php $j = 0; foreach ($langList as $key => $row):
									if($row['id'] == $hadithLangInfo[$j]['langId']){
										$translateId = $hadithLangInfo[$j]['id'];
										$translateTitle = $hadithLangInfo[$j]['title'];
										$translateStatus = $hadithLangInfo[$j]['status'];
										$translateDescription = $hadithLangInfo[$j]['description'];
									}
									?>
									<div class="tab-pane fade <?php echo ($key !== 0) ?: 'show active';?>" id="<?=$row['title']?>" role="tabpanel" aria-labelledby="<?=$row['title']?>-tab">
										...
										<input type="text" hidden name="translateId-<?=++$key?>" value="<?=$translateId?>">
										<div class="form-group">
										...
										<div class="form-group">
											...
											<select name="stts-<?=$key?>" id="status-<?=$row['id']?>" class="form-control">
												...
												<option value="DELETED">DELETED</option>
												<option value="DISABLED">DISABLED</option>
												<option style="background-color: #00A000; color:#fff;" value="<?=$translateStatus?>" selected><?=$translateStatus?></option>
											</select>
										</div>
										<div class="form-group">
